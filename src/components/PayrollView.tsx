    ...
    <Tabs defaultValue="parametres" className="space-y-4">
      ...
      <TabsContent value="mensuelles" className="space-y-4">
        ...
        <DataFrame loading={isLoading} empty={!data?.payslips.length} columns={["Mois", "Fiche", "Auteur", "Dernier modificateur", "Statut", "Actions"]}>
          {(data?.payslips ?? []).map((p) => <TableRow key={p.id}><TableCell>{MONTHS[p.mois - 1]} {p.annee}</TableCell><TableCell><Button variant="ghost" size="icon" title="Consulter la fiche" onClick={() => viewPayslip(p)}><Eye className="size-4" /></Button></TableCell><TableCell>{p.auteur_nom ?? "—"}</TableCell><TableCell>{p.modificateur_nom ?? "—"}</TableCell><TableCell><StatusBadge status={p.statut} /></TableCell><TableCell className="whitespace-nowrap">{isAdmin && NEXT_STATUS[p.statut] ? <Button variant="outline" size="sm" onClick={() => updatePayslip.mutate({ id: p.id, statut: NEXT_STATUS[p.statut] })}><CheckCircle2 className="mr-2 size-4" />{NEXT_STATUS[p.statut]}</Button> : null}{isAdmin && p.statut !== "Annulé" && p.statut !== "Payé" ? <Button variant="ghost" size="sm" className="text-destructive" onClick={() => updatePayslip.mutate({ id: p.id, statut: "Annulé" })}><XCircle className="mr-2 size-4" />Annuler</Button> : null}</TableCell></TableRow>)}
        </DataFrame>
      </TabsContent>
    </Tabs>
